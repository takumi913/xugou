...
const schemas: Record<string, Schema> = {
  ...
  PublicAgent: { type: "object", required: ["id", "name", "status", "created_at", "updated_at", "metrics"], properties: { id: { type: "integer" }, name: { type: "string" }, status: { type: "string", enum: ["active", "inactive", "connecting", "unknown"] }, hostname: { type: ["string", "null"] }, os: { type: ["string", "null"] }, version: { type: ["string", "null"] }, region: { type: ["string", "null"] }, city: { type: ["string", "null"], maxLength: 128 }, region_name: { type: ["string", "null"], maxLength: 128 }, map_latitude: { type: ["number", "null"], minimum: -90, maximum: 90 }, map_longitude: { type: ["number", "null"], minimum: -180, maximum: 180 }, created_at: { type: "string" }, updated_at: { type: "string" }, traffic_limit_gb: { type: ["number", "null"] }, traffic_reset_day: { type: ["integer", "null"] }, traffic_calc_type: { type: ["string", "null"], enum: ["sum", "rx", "tx", null] }, metrics: { oneOf: [ref("PublicAgentMetric"), { type: "null" }] } } },
...
for (const [moduleName, variable] of routeFiles) {
  ...
  for (const match of source.matchAll(expression)) {
    ...
    const operation = `${match[1]} ${route}`;
    // OpenAPI paths describe HTTP responses; WebSocket upgrade contracts are
    // verified by the AgentRoom runtime and realtime architecture suites.
    if (!transportOnlyRoutes.has(operation)) implemented.add(operation);
  }
}
for (const route of [
