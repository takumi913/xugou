...
const agentV2UpdateFields = {
    ...
    collect_interval_seconds: z.coerce.number().int().min(1).max(86400).optional(),
    report_interval_seconds: z.coerce.number().int().min(1).max(86400).optional(),
...
const agentLegacyImportItemSchema = z
  .object({
    ...
    os: z.string().trim().max(128).nullable().optional(),
    version: z.string().trim().max(128).nullable().optional(),
    collect_interval: z.number().int().min(1).max(86400).optional(),
    report_interval: z.number().int().min(1).max(86400).optional(),
...
export const agentV4ReportSchema = z
  .object({
    ...
    report_interval_seconds: z.number().int().min(1).max(86400).optional(),
